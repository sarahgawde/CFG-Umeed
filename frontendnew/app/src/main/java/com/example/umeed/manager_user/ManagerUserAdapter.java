...
public class ManagerUserAdapter extends RecyclerView.Adapter<ManagerUserAdapter.ManagerViewHolder> {
    ...
    public ManagerUserAdapter(Context context, ArrayList<ManagerUserResponseModel.Data.Message> subcategoryModels) {
    ...
    public void onBindViewHolder(@NonNull ManagerViewHolder holder, int position) {
        holder.tvCategoryTitle.setText(subcategoryModels.get(position).getUserAcc().getFirstName());
        holder.tvmobileNumber.setText(subcategoryModels.get(position).getUserAcc().getUsername());
